...
- UX polish rounds 6–10 (branch `fix/ux-ui-polish`):
  ...
  - `panelStyles.ts`: `.companion-block p { margin: 0 }` resets browser-default paragraph margins inside tight grid cells; `.companion-block .status-actions { margin-top: var(--space-2) }` keeps the action row consistently spaced; `.companion-block .compact-list { padding-left: 12px }` tightens list indent inside grid cells.
