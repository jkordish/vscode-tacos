...
function buildDemoResumeSummary(now = Date.now()): ResumeSummary {
  return {
    ...
    inferredIntent: 'Wire percolation ranking into surface broker and add unit coverage',
    intentOverridden: false,
    nextSteps: [
      'Finish `scoreByRecency` weight in `src/percolation/ranking.ts` — left off at line 94.',
      'Run `npm test -- --testPathPattern=percolationRanking` to verify the new weight.',
      'Update `src/percolation/surfaceBroker.ts` to call `rankCandidates()` before promotion.',
    ],
    nextStepEvidenceIds: [
      ['file:ranking', 'git:ranking-diff'],
      ['task:verify'],
      ['file:broker', 'branch:feat'],
    ],
    lastActionLabel:
      'Added `scoreByRecency` stub in `src/percolation/ranking.ts` — weight factor left as TODO.',
    lastActionContext: 'feat/percolation-surface-v2 · last edit ~42 min ago',
    lastActionEvidenceId: 'file:ranking',
    doneSinceLastResume: [
      'Added `RankedCandidate` type and `rankCandidates()` skeleton in `ranking.ts`',
      'Wired `PercolationMemory` signal cache reset on partition switch',
      'Covered `promptGate` suppress/allow paths — 14 new unit assertions',
    ],
    changesSinceLastResume: [
      '4 files modified: `ranking.ts`, `surfaceBroker.ts`, `memory.ts`, `types.ts`',
      '`test/percolationRanking.test.ts` — 6 passing, 2 skipped (recency weight pending)',
    ],
    pendingBlocked: [
      '`percolationDecisionMatrix` integration test is flaky on CI — likely a timing race in signal cache reset',
    ],
    recommendedFirstAction:
      'Open `src/percolation/ranking.ts` at line 94 and implement the `scoreByRecency` weight.',
    lowConfidence: false,
    longGap: false,
    resumeGapMinutes: 42,
    candidateIntents: [
      'Wire percolation ranking into surface broker and add unit coverage',
      'Fix flaky percolationDecisionMatrix integration test',
      'Write docs for percolation policy API',
    ],
    mode: 'coding',
    topFiles: [
      'src/percolation/ranking.ts',
      'src/percolation/surfaceBroker.ts',
      'src/percolation/memory.ts',
      'test/percolationRanking.test.ts',
    ],
    links: [],
    currentBranch: 'feat/percolation-surface-v2',
    previousBranch: 'main',
    evidenceCatalog: [
      {
        id: 'file:ranking',
    ...
    detailsMarkdown: [
      ...
      '- `src/percolation/surfaceBroker.ts` — stub call to `rankCandidates()` added but not yet wired to promotion path.',
      '- `src/percolation/memory.ts` — signal cache now resets on partition switch (covered).',
      '- `src/percolation/types.ts` — exported `RankedCandidate` and `RankingWeights` interfaces.',
      '',
      '### Blocker',
      '`percolationDecisionMatrix` integration test is intermittently failing on CI. Likely a timing race in signal-cache reset between test cases. Repros 1-in-3 locally with `--runInBand` disabled.',
      '',
      '### Safe next action',
      'Implement `scoreByRecency` in `ranking.ts` (line 94), run unit tests, then wire `surfaceBroker.ts`. Do **not** rebase onto main until the integration test is stable.',
      '',
      '### Restore steps',
      '1. Open `src/percolation/ranking.ts` → line 94',
      '2. `npm test -- --testPathPattern=percolationRanking`',
      '3. Open `src/percolation/surfaceBroker.ts` and wire `rankCandidates()` into promotion path',
    ].join('\n'),
    codexPrompt:
      'I am resuming work on feat/percolation-surface-v2. My intent is to wire percolation ranking into the surface broker. I left off implementing scoreByRecency in src/percolation/ranking.ts at line 94. The next safe action is to implement that weight, run unit tests, then wire surfaceBroker.ts. There is a flaky integration test (percolationDecisionMatrix) that I should not block on. Summarize the safest next step and flag any risks.',
