...
- Companion Home includes a one-click `Review AI payload preview` deep-link for the currently surfaced context.
      ...
      - **Evidence tab grouped rendering (P21):** The Evidence tab defaults to `recent` mode (last 5 anchors, sorted newest-first) every time the panel is opened. A four-button group-mode toggle bar lets users switch between `recent`, `by-file`, `by-time`, and `by-action`. The active mode is stored in `state.panelEvidenceGroupMode` (`EvidenceGroupMode`), reset to `'recent'` on each new panel creation, and driven by the `setEvidenceGroupMode` webview message. The `tacos.evidence.granularity` setting (`coarse` / `medium` / `fine`) controls anchor density within each mode. Grouping logic lives in pure functions `selectRecentAnchors`, `groupTimelineByFile`, and `groupTimelineByTimeBucket` in `src/timeline.ts`; rendering in `renderGroupedEvidenceTab` (`src/webview/panelCards.ts`). Integration test probe: `tacos.__test.setEvidenceGroupMode`.
